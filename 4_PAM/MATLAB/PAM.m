...
msg = 0.5 * sin(2*pi*f_msg*t) + 0.5; % Message between 0 and 1 for PAM
...
%% 4. Generate Original PAM Signal
pam_original = zeros(size(t));
for i = 1:length(pulse_times)-1
    msg_idx = find(t >= pulse_times(i), 1);
    if msg_idx <= length(msg)
        idx = (t >= pulse_times(i)) & (t < pulse_times(i) + pulse_width);
        pam_original(idx) = msg(msg_idx);
    end
end
...
% Quantize PAM for error calculation (e.g., 4 levels for simplicity)
quant_levels = 4;
quant_edges = linspace(0, 1, quant_levels+1);
pam_original_quant = zeros(size(pam_original));
for i = 1:length(pam_original)
    if pam_original(i) > 0
        [~, idx] = min(abs(pam_original(i) - quant_edges));
        pam_original_quant(i) = quant_edges(max(1, idx-1));
    end
end

figure('Name', 'PAM with Noise Analysis');
for i = 1:length(SNR_levels)
    SNR = SNR_levels(i);
    msg_noisy = awgn(msg, SNR, 'measured');
    msg_noisy = max(0, min(1, msg_noisy)); % Clip to [0, 1] for valid PAM
    
    pam_noisy = zeros(size(t));
    for j = 1:length(pulse_times)-1
        msg_idx = find(t >= pulse_times(j), 1);
        if msg_idx <= length(msg_noisy)
            idx = (t >= pulse_times(j)) & (t < pulse_times(j) + pulse_width);
            pam_noisy(idx) = msg_noisy(msg_idx);
        end
    end
    
    % Quantize noisy PAM for error calculation
    pam_noisy_quant = zeros(size(pam_noisy));
    for j = 1:length(pam_noisy)
        if pam_noisy(j) > 0
            [~, idx] = min(abs(pam_noisy(j) - quant_edges));
            pam_noisy_quant(j) = quant_edges(max(1, idx-1));
        end
    end
    
    error_rate = sum(pam_noisy_quant ~= pam_original_quant) / N;
    ...
    plot(t*1000, pam_noisy, 'k', 'LineWidth', 1.5);
    title(['PAM Output (SNR = ', num2str(SNR), ' dB)']);
    xlabel('Time (ms)');
    ylabel('PAM');
    ylim([-0.2 1.2]);
    grid on;
...
subplot(3,1,3);
plot(t*1000, pam_original, 'k', 'LineWidth', 1.5);
title('PAM Output Signal (No Noise)');
xlabel('Time (ms)');
ylabel('PAM');
ylim([-0.2 1.2]);
grid on;
...
output_folder = fullfile(pwd, 'PAM_Outputs');
